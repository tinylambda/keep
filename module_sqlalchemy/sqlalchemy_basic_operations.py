...
print(
    "users.c.name + users.c.fullname =>",
    (users.c.name + users.c.fullname).compile(bind=engine),
)
print(
    "If use mysql =>",
    (users.c.name + users.c.fullname).compile(
        bind=create_engine("mysql+mysqlconnector://")
    ),
)
...
for item in connection.execute(s, x="m", y="z", e1="%@aol.com", e2="%@qq.com"):
    print(item)
...
)  # automatically detect the ON clause based on the ForeignKey
...
print("Use filter")
for item in (
    session.query(User).filter(User.name != "felix").filter(User.name != "demo1")
):
    print(item)
