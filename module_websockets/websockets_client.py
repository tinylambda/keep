...
async def simple_echo_client(uri: str, stdin_q: asyncio.Queue, stdout_q: asyncio.Queue):
    ...
    async with websockets.connect(uri) as websocket:
        ...
        stdin_q_input_task: asyncio.Task = asyncio.get_event_loop().create_task(stdin_q.get())
        stdin_q_input_task.add_done_callback(lambda x: print('1>>> '))
        ws_recv_task: asyncio.Task = asyncio.get_event_loop().create_task(websocket.recv())
...
try:
    event_loop.run_until_complete(simple_echo_client(uri='ws://localhost:20000/ws/chat/cc/', stdin_q=in_q, stdout_q=out_q))
finally:
    event_loop.close()
