...
if __name__ == "__main__":
    ...
    threads = [
        threading.Thread(
            name="Reader 1", target=reader, args=(db_filename, isolation_level, ready)
        ),
        threading.Thread(
            name="Reader 2", target=reader, args=(db_filename, isolation_level, ready)
        ),
        threading.Thread(
            name="Writer 1", target=writer, args=(db_filename, isolation_level, ready)
        ),
        threading.Thread(
            name="Writer 2", target=writer, args=(db_filename, isolation_level, ready)
        ),
    ]
