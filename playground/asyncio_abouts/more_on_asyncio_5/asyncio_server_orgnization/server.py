...
class Server(metaclass=ServerMeta):
    ...
    async def listen(cls):
        ...
        while True:
            ...
            services = [''.join(random.choices(string.ascii_lowercase, k=2)) for _ in range(random_i)]
            cls.SERVICES.clear()
    ...
    async def print_services(cls):
        while True:
            logging.info('current services: %s, user001 goto service: %s', cls.SERVICES, cls.HASH_RING.get_node('user001'))
