...
async def consume(executor, q):
    ...
    loop = asyncio.get_running_loop()
    asyncio.ensure_future(loop.run_in_executor(executor, consume_sync, q, loop), loop=loop)
...
async def handle_message(msg: PubSubMessage):
    ...
    results = await asyncio.gather(save(msg), restart_host(msg), return_exceptions=True, loop=loop)
...
def main():
    ...
    for s in signals:
        loop.add_signal_handler(s, lambda s=s: asyncio.create_task(shutdown(loop, executor, sig=s)))
