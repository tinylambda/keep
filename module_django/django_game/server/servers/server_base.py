...
class ServerBase(EntityESOperationMixin):
    ...
    async def handle_task_input(self, task_input: bytes):
        header_bytes, server, action, task_input_body = task_input.split(
            GameConsumer.PACK_DELIMITER
        )
        ...
        if action_name == "create":
            ...
            if task_id:
                ...
                if task_instance is None:
                    logging.error(
                        f"task instance with {self.ID_NAME} = {task_id} cannot be found or created"
                    )
                else:
    ...
    async def core_loop(
        self, task_data: typing.Dict, result_q: asyncio.Queue, output_q: asyncio.Queue
    ):
    ...
    async def server_main(self):
        ...
        self.main_tasks: typing.List[asyncio.Task] = [
            f() for f in self.main_task_callables
        ]
