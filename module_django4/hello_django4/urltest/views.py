...
class UrlTestRePathView(View):
    def get(self, request, year, month):
        ...
        return JsonResponse({'year': year, 'month': month, 'year_type': str(type(year))})


class UrlNestedArguments(View):
    def get(self, request, page_number):
        # visit http://127.0.0.1:8000/books/comments/page-222 to test
        return JsonResponse({'page_number': page_number, 'page_number_type': str(type(page_number))})
...
class UrlIncludeViewOne(View):
    def get(self, request, page_slug, page_id):
        ...
        return JsonResponse({'view': 'UrlIncludeViewOne', 'page_slug': page_slug, 'page_id': page_id})


class UrlIncludeViewTwo(View):
    def get(self, request, page_slug, page_id):
        # visit http://127.0.0.1:8000/books/test-123/edit/
        return JsonResponse({'view': 'UrlIncludeViewTwo', 'page_slug': page_slug, 'page_id': page_id})
