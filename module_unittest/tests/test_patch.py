...
class TestValidateOption(unittest.TestCase):
    ...
    def test_stdout_stream(self, warn):
        self.assertRaises(ValueError, validate_option, "stdout_stream", "something")
        self.assertRaises(ValueError, validate_option, "stdout_stream", {})
        validate_option("stdout_stream", {"class": "MyClass"})
        validate_option("stdout_stream", {"class": "MyClass", "my_option": "1"})
        validate_option("stdout_stream", {"class": "MyClass", "refresh_time": 1})
        warn.assert_called_once()

    @patch("warnings.warn")
    def test_stderr_stream(self, warn):
        self.assertRaises(ValueError, validate_option, "stderr_stream", "something")
