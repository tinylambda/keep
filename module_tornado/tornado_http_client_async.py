...
if __name__ == '__main__':
    ...
    loop.add_future(asyncio.ensure_future(asynchronous_fetch(test_url)), functools.partial(cb, use_loop=loop))
    loop.start()

    loop = tornado.ioloop.IOLoop.current()
    loop.add_future(asynchronous_fetch_old(test_url), functools.partial(cb, use_loop=loop))
    loop.start()
