...
class SubjectTTLExtractor:
    KEY_SEP = '-'
    KEY_NAMES = ['dummy', 'subject_id', 'user_type', 'user_id', 'media_type', 'timestamp']
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description="SubjectTTLExtractor to copy data from HBase to a local file")
    parser.add_argument('--date', action='store', dest='date', default=one_day_ago_date_str)
    parser.add_argument('--filename', action='store', dest='filename', default=default_filename_str)
    args = parser.parse_args()

    ste = SubjectTTLExtractor()
    date = '2020-03-10'
    filename = args.filename
    subject_ids = ['26347420',
                   # '27875044',
                   # '27540530',
                   ]
